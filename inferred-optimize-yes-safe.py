...
# Safety data also has an inferred partition (unlike inferred-optimize-no-safe.py)

if __name__ == "__main__":

    parser = argparse.ArgumentParser()
    parser.add_argument('--config', type=str, default='configs/default_inferred_yes_safe.yaml')
    ...
    parser.add_argument('--safety_inferred_proportion',  type=float, default=None)
    ...
    SAFETY_INFERRED_PROPORTION  = cfg.get('safety_inferred_proportion', INFERRED_PROPORTION)
    ...
    assert CONSTRAINT_TYPE in ['students-ttest', 'welchs-ttest'], f"Invalid constraint type: {CONSTRAINT_TYPE}"
    assert DISPARITY_TYPE in ['reward', 'action'], f"Invalid disparity type: {DISPARITY_TYPE}"

    wandb.init(
        project="group-fair-bandits",
        name=f"{DATSET_NAME}-{SENSITIVE_ATTRIBUTE}-{MODE}-{CONSTRAINT_TRUNC}-{BEHAVIOR_POLICY_TYPE}-yes-safe",
        config={
            ...
            "safety_inferred_proportion": SAFETY_INFERRED_PROPORTION,
            "FPR":                        FPR,
            "FNR":                        FNR,
        },
    )

    assert BEHAVIOR_POLICY_TYPE in ['random', 'tweak1', 'mixed'], f"Invalid behavior policy type: {BEHAVIOR_POLICY_TYPE}"
    ...
    # -------------------------------------------------------------------------
    # Train split: ground-truth and inferred partitions
    # -------------------------------------------------------------------------
    ...
    print(f"Train inferred flips — FP (0→1): {n_fp}, FN (1→0): {n_fn}, total: {flip_mask.sum()}")
    train_data['sensitive'] = pd.Series(sensitive_noisy, index=train_data['sensitive'].index)
    is_inferred_train = np.zeros(n_train, dtype=bool)
    is_inferred_train[inferred_idx] = True
    train_data['is_inferred'] = pd.Series(is_inferred_train, index=train_data['sensitive'].index)

    # Group sizes from ground-truth train partition only
    gt_train_sensitive = sensitive_noisy[~is_inferred_train]
    train_grp_1_size   = int(gt_train_sensitive.sum())
    train_grp_2_size   = int((~is_inferred_train).sum()) - train_grp_1_size
    print(f"Train GT partition — grp1 (sens=1): {train_grp_1_size}, grp2 (sens=0): {train_grp_2_size}")

    # -------------------------------------------------------------------------
    # Safety split: ground-truth and inferred partitions
    # -------------------------------------------------------------------------
    n_safety            = len(safety_data['context'])
    n_safety_inferred   = int(n_safety * SAFETY_INFERRED_PROPORTION)
    safety_inferred_idx = np.random.choice(n_safety, size=n_safety_inferred, replace=False)
    print(f"Safety split — ground-truth: {n_safety - n_safety_inferred}, inferred: {n_safety_inferred} "
          f"(proportion={SAFETY_INFERRED_PROPORTION}, fpr={FPR}, fnr={FNR})")

    safety_sensitive_noisy = safety_data['sensitive'].values.ravel().copy().astype(int)
    safety_inferred_orig   = safety_sensitive_noisy[safety_inferred_idx]
    safety_flip_probs      = np.where(safety_inferred_orig == 0, FPR, FNR)
    safety_flip_mask       = np.random.random(n_safety_inferred) < safety_flip_probs
    safety_sensitive_noisy[safety_inferred_idx[safety_flip_mask]] = 1 - safety_sensitive_noisy[safety_inferred_idx[safety_flip_mask]]
    s_fp = int(safety_flip_mask[safety_inferred_orig == 0].sum())
    s_fn = int(safety_flip_mask[safety_inferred_orig == 1].sum())
    print(f"Safety inferred flips — FP (0→1): {s_fp}, FN (1→0): {s_fn}, total: {safety_flip_mask.sum()}")
    safety_data['sensitive'] = pd.Series(safety_sensitive_noisy, index=safety_data['sensitive'].index)
    is_inferred_safety = np.zeros(n_safety, dtype=bool)
    is_inferred_safety[safety_inferred_idx] = True
    safety_data['is_inferred'] = pd.Series(is_inferred_safety, index=safety_data['sensitive'].index)

    # Group sizes from ground-truth safety partition only
    gt_safety_sensitive    = safety_sensitive_noisy[~is_inferred_safety]
    safety_grp_1_size      = int(gt_safety_sensitive.sum())
    safety_grp_2_size      = int((~is_inferred_safety).sum()) - safety_grp_1_size
    print(f"Safety GT partition — grp1 (sens=1): {safety_grp_1_size}, grp2 (sens=0): {safety_grp_2_size}")

    # All 4 combinations of (sensitive, is_inferred) in safety split
    n_safety_grp1_ground   = int((safety_sensitive_noisy[~is_inferred_safety] == 1).sum())  # sens=1, GT
    n_safety_grp2_ground   = int((safety_sensitive_noisy[~is_inferred_safety] == 0).sum())  # sens=0, GT
    n_safety_grp1_inferred = int((safety_sensitive_noisy[ is_inferred_safety] == 1).sum())  # sens=1, inferred
    n_safety_grp2_inferred = int((safety_sensitive_noisy[ is_inferred_safety] == 0).sum())  # sens=0, inferred
    print(f"Safety counts — GT: grp1={n_safety_grp1_ground}, grp2={n_safety_grp2_ground} | "
          f"Inferred: grp1={n_safety_grp1_inferred}, grp2={n_safety_grp2_inferred}")
    ...
    for epoch in epoch_iter:
        ...
        for i, b_sidx in batch_iter:
            ...
            if MODE == 'seldonian' and (sensitive_mask.sum().item() == 0 or sensitive_mask.sum().item() == BATCH_SIZE):
                print(f"Warning: One of the sensitive groups has no samples in this batch. Skipping.")
                continue

            if MODE == 'seldonian' and (inferred_mask.sum().item() == 0 or inferred_mask.sum().item() == BATCH_SIZE):
                print(f"Warning: One of the inferred groups has no samples in this batch. Skipping.")
                continue
            ...
            constraint_kwargs = {
                'fail_prob':                    FAIL_PROB,
                'epsilon':                      EPSILON,
                'safety_data_grp_1_size':       safety_grp_1_size,
                'safety_data_grp_2_size':       safety_grp_2_size,
                'constraint_type':              CONSTRAINT_TYPE,
                'disparity_type':               DISPARITY_TYPE,
                ...
                'fpr':                          FPR,
                'fnr':                          FNR,
                # safety split counts — all 4 (sensitive, is_inferred) combinations
                'n_safety_grp1_ground':   n_safety_grp1_ground,    # sens=1, GT
                'n_safety_grp2_ground':   n_safety_grp2_ground,    # sens=0, GT
                'n_safety_grp1_inferred': n_safety_grp1_inferred,  # sens=1, inferred
                'n_safety_grp2_inferred': n_safety_grp2_inferred,  # sens=0, inferred
            }

            if MODE == 'seldonian':
                # -----------------------------------------------------------------
                # PLACEHOLDER: replace with inferred-safety-aware constraint
                # The constraint function here must account for label noise in both
                # the train and safety inferred partitions.
                # -----------------------------------------------------------------
            ...
            if step_idx % EVAL_FREQ == 0:
                ...
                if MODE == 'seldonian':
                    ...
                    if DISPARITY_TYPE == 'reward':
                        ...
                        raise ValueError(f"Invalid disparity type: {DISPARITY_TYPE}")
    ...
    # Perform Safety Test
    # TODO: replace with inferred-safety-aware safety test
