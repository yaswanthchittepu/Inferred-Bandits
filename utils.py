...
def bound_prop_ucb_constraint_no_safety(is_reward, is_action, output_probs_a, sensitive_mask, inferred_mask, log_lambda1, log_lambda2, constraint_kwargs):
    ...
    # Total Stdev Term
    stdev_combined = (torch.exp(log_lambda1).detach().item()+torch.exp(log_lambda2).detach().item()) * ((group_1_stdev_term_ground) + (group_2_stdev_term_ground))
    ...
    with torch.no_grad():
        ...
        ucb_g1_v_g2 = mean_terms_g1 - mean_terms_g2 + K1*group_1_is_metric_ground_std + K2*group_2_is_metric_ground_std
        ucb_g2_v_g1 = mean_terms_g2 - mean_terms_g1 + K1*group_1_is_metric_ground_std + K2*group_2_is_metric_ground_std
